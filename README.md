...
- **画像生成** — V5 / V4.5 / V4 / V3 系モデル、解像度プリセット、ステップ数・プロンプトの強さ・
  シード・サンプラーなどの調整。V5 では背景の透過（`transparent background` タグ＋アルファ付き
  出力）も指定できる。ノイズスケジュールとバリエーション強化（Variety+）は V5 に無いので、
  V5 では出ない
...
- **画像解析** — 生成した画像が誰の絵柄に近いかを手元のモデル（Kaloscope 2.0）で調べる。
  モデルは初回に設定フォルダへダウンロードする
...
| onnxruntime-node（画像解析。Windows 分の DLL） | 132 MB |
| web の静的ビルド | 1.2 MB |

画像解析のモデル本体（Kaloscope 2.0、734 MB）は同梱しない。解析ダイアログの
ボタンで初回にダウンロードし、`~/.config/nai-desktop-studio/models/` に置く。
